...
from . import views_bp, MaterialTracking, db, UPLOAD_FOLDER
...
def material_tracking_detection(bulk_id):
    if request.method == 'POST':
        ...
        if picture:
            ...
            picture_dir = os.path.join(UPLOAD_FOLDER, str(bulk_id))
        ...
        return redirect(url_for('views.material_tracking', bulk_id=bulk_id, date_tracked=date_tracked_str,
...
def material_tracking(bulk_id):
    ...
    if request.method == 'POST':
        ...
        new_material_tracking = MaterialTracking(bulk_id=bulk_id, date_tracked=date_tracked, quantity=quantity,
                                                 responsible_person=responsible_person, picture_url=picture_url,
                                                 picture_detection_url=picture_detection_url)
